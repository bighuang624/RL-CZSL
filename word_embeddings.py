...
def load_word_embeddings(vocab, args):
    if args.emb_type == 'glove':
        embeds = load_glove_embeddings(vocab, args)
    elif args.emb_type == 'fasttext':
        embeds = load_fasttext_embeddings(vocab, args)
    elif args.emb_type == 'word2vec':
        embeds = load_word2vec_embeddings(vocab, args)
        ...
        embeds1 = load_fasttext_embeddings(vocab, args)
        embeds2 = load_glove_embeddings(vocab, args)
        embeds = torch.cat([embeds1, embeds2], dim = 1)
        print('Combined embeddings are ', embeds.shape)
        ...
        embeds1 = load_glove_embeddings(vocab, args)
        embeds2 = load_word2vec_embeddings(vocab, args)
        embeds = torch.cat([embeds1, embeds2], dim = 1)
        print('Combined embeddings are ', embeds.shape)
        ...
        embeds1 = load_fasttext_embeddings(vocab, args)
        embeds2 = load_word2vec_embeddings(vocab, args)
        embeds3 = load_glove_embeddings(vocab, args)
...
def load_fasttext_embeddings(vocab, args):
    custom_map = {
        ...
        'eiffel_tower' : 'tower',
        # ? below is for RL-CZSL-ACT
        'scoope': 'scoop',
        'eatat': 'eat_at',
        'drinkwith': 'drink_with',
        'brushwith': 'brush_with',
        'typeon': 'type_on',
        'siton': 'sit_on',
        'standon': 'stand_on',
        'cutwith': 'cut_with',
        'standunder': 'stand_under',
        'lieon': 'lie_on',
        'sitat': 'sit_at',
    ...
    ft = fasttext.load_model(args.data_folder+'/fast/cc.en.300.bin')
    embeds = []
    for k in vocab:
        if '_' in k:
            ks = k.split('_')
...
def load_word2vec_embeddings(vocab, args):
    ...
    model = models.KeyedVectors.load_word2vec_format(
        args.data_folder+'/w2v/GoogleNews-vectors-negative300.bin', binary=True)
...
def load_glove_embeddings(vocab, args):
    '''
    Inputs
    ...
    '''
    vocab = [v.lower() for v in vocab]
    emb_file = args.data_folder+'/glove/glove.6B.300d.txt'
