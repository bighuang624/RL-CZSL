...
class SUNClassMMDataset(ClassDataset):
        ...
        if self.semantic_type is not None:
            for single_semantic_type in self.semantic_type:
                if single_semantic_type not in self.semantic_type_limitation:
                    raise ValueError('Non-supported Semantic Type for SUN.')
    ...
    def __getitem__(self, index):
        ...
        semantics = {}
        if 'image_attributes' in self.semantic_type:
            semantics['image_attributes'] = self.images_attributes_dict[class_name]

        return SUNMMDataset(index, data, class_name, self.semantic_type, semantics,
...
class SUNMMDataset(Dataset):
    def __init__(self, index, data, class_name, semantic_type, semantics,
        ...
        self.semantic_type = semantic_type
        self.semantics = semantics    # semantics['image_attributes'] is a dict
    ...
    def __getitem__(self, index):
        ...
        if 'image_attributes' in self.semantic_type:
            image_attribute = self.semantics['image_attributes'][image_name]
            image_attribute = torch.Tensor(image_attribute)    # to Tensor
            self.content['image_attributes'] = image_attribute
