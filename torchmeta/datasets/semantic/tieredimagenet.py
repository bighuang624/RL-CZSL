...
class TieredImagenetMM(CombinationMetaDataset):
    """
    ...
    def __init__(self, root, num_classes_per_task=None, meta_train=False,
        ...
        super(TieredImagenetMM, self).__init__(dataset, num_classes_per_task,
...
class TieredImagenetClassDataset(ClassDataset):
        ...
        if self.semantic_type is not None:
            for single_semantic_type in self.semantic_type:
                if single_semantic_type not in self.semantic_type_limitation:
                    raise ValueError('Non-supported Semantic Type for TieredImagenet.')
    ...
    def __getitem__(self, index):
        ...
        semantics = {}
        if 'class_name_embeddings' in self.semantic_type:
            semantics['class_name_embeddings'] = self.class_name_embedding_dict[specific_class_name]

        return TieredImagenetDataset(index, data, general_class_name,
            specific_class_name, self.semantic_type, semantics,
    ...
    def get_class_name_embedding_dict(self):
        ...
        # delete all ',' before splitting
        # only specific_class_name is used for label embedding
        class_name_embedding_dict = {}
        for specific_class_name in self.labels:
            if specific_class_name not in class_name_embedding_dict.keys():
                class_names = specific_class_name.lower().split(', ')
                tmpv = np.zeros(self.embedding_dim)
                tmpl = []
                count = 0
                class_name_words = class_names[0].split(' ')
                if class_names[0]=='limpkin' or class_names[0]=='otterhound' or class_names[0]=='barracouta' or class_names[0]=='lycaenid':
                    class_name_words = class_names[1].strip().split(' ')
                for word in class_name_words:
                    if word in word_embedding_dict.keys():
                        ...
                        count += 1
                if count != 0:
                    class_name_embedding_dict[specific_class_name] = tmpv / count
                else:
                    class_name_embedding_dict[specific_class_name] = np.random.rand(self.embedding_dim)*2-1    # here follows AM3 that does not use zero vector
...
class TieredImagenetDataset(Dataset):
    def __init__(self, index, data, general_class_name, specific_class_name, 
                 semantic_type, semantics, transform=None, target_transform=None):
