...
class TieredImagenetNormalDataset(torch.utils.data.Dataset):
    folder = 'tieredimagenet'
    # Google Drive ID from https://github.com/renmengye/few-shot-ssl-public
    gdrive_id = '1g1aIDy2Ar_MViF2gDXFYDBTR-HYecV07'
    tar_filename = 'tiered-imagenet.tar'
    ...
    def __init__(self, root, split, transform=None, target_transform=None, download=False):
        super(TieredImagenetNormalDataset, self).__init__()
        ...
        self.split_filename = os.path.join(self.root,
            self.filename.format(self.split))
        self.split_filename_labels = os.path.join(self.root,
            self.filename_labels.format(self.split))

        self._data_file = h5py.File(self.split_filename, 'r')
        # self._data contains all samples from validation set
        # self._labels corresponds to self._data
        self._data, self._labels = self._get_data()
    ...
    def _get_data(self):
        # not consider general_class_name
        data = []
        labels = []
        i_label = 0
        for label in self._data_file['datasets']:
            data_per_class = self._data_file['datasets'][label]
            for data_item in data_per_class:
    ...
    def __getitem__(self, index):
        image = Image.open(io.BytesIO(self._data[index])).convert('RGB')
