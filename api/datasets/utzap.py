...
def reorganize(root, folder):
    ...
    print("reorganizing ...", root)
    os.makedirs(os.path.join(root,'images'))
    data = torch.load(os.path.join(root,'metadata.t7'))
    for instance in tqdm.tqdm(data):
        image, attr, obj = instance['_image'], instance['attr'], instance['obj']
        old_file = '%s/%s/%s'%(root, folder, image)
    ...
    shutil.move(root+'/images', os.path.join(root, folder))

class UTZap(Dataset):

    def __init__(self, root, meta_train=False, meta_val=False, meta_test=False,
                 meta_split=None, transform=None, download=False):
        super(UTZap, self).__init__()

        self.folder = "utzap"  # Q: This should go under the same folder as cub, i.e. assets
        self.filename = '{0}_data.hdf5'
        self.filename_labels = '{0}_labels.json'
        self.image_folder = 'ut-zap50k-images/images'
    ...
    def download(self):
        ...
        if not os.path.isdir(image_folder):
            reorganize(self.root, self.image_folder.split('/')[0])
