...
class ACTIONOBJ(Dataset):

    def __init__(self, root, meta_train=False, meta_val=False, meta_test=False,
                 meta_split=None, transform=None, download=False, num_negs=None):
        super(ACTIONOBJ, self).__init__()

        self.folder = "action_obj"  # Q: This should go under the same folder as cub, i.e. assets
        ...
        if self.meta_split == 'train':
            self.meta_split_name = 'train_hico'
        elif self.meta_split == 'val':
            self.meta_split_name = 'val_visualgenome'
        elif self.meta_split == 'test':
            self.meta_split_name = 'test_imsitu'
    ...
    def get_label2idx_dict(self):
        ...
        all_labels_1 = set()
        all_labels_2 = set()
        for cur_meta_split_name in ['train_hico', 'val_visualgenome', 'test_imsitu']:
    ...
    def download(self):
        ...
        for split in ['train_hico', 'val_visualgenome', 'test_imsitu']:
            filename = os.path.join(self.root, self.filename.format(split))
            if os.path.isfile(filename):
                continue

            if split == 'train_hico':
                image_folder = os.path.join(self.root, "hico/hico_20160224_det/new_images")
            elif split == 'val_visualgenome':
                image_folder = os.path.join(self.root, "VG_100K")
            elif split == 'test_imsitu':
                image_folder = os.path.join(self.root, "imsitu")
            else:
